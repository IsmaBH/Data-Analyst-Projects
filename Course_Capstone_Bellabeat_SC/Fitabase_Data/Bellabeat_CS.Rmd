...
sleep$time <- format(sleep$date, format = "%H:%M:%S")
sleep$ShortDate <- format(sleep$date, format = "%m/%d/%y")
stress$Time=as.POSIXct(stress$Time, format="%m/%d/%Y %I:%M:%S", tz="")
stress$hour <- format(stress$Time, format = "%H:%M:%S")
stress$ShortDate <- format(stress$Time, format = "%m/%d/%y")
#summarise(group_by(sleep,sleep$Id), sum(sleep$hour))
#sleep$date <- format(sleep$SleepDate, format = "%m/%d/%y")
```
